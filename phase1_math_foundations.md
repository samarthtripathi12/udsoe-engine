...
- Determine **controllable vs exogenous variables**  
...
$$
\mathcal{L}(x, \lambda, \mu) = U_i(x_i) + \sum_j \lambda_j g_j(x) + \sum_k \mu_k h_k(x)
$$
...
\nabla_x \mathcal{L} = 
\begin{bmatrix}
\frac{\partial U_i}{\partial x_1} + \lambda \frac{\partial g_1}{\partial x_1} \\[2mm]
...
## 9. References & Mini-Derivations
1. **Boyd, S. & Vandenberghe, L.** *Convex Optimization*, Cambridge University Press, 2004  
   - Gradient-based optimization, Lagrangian methods, convex constraints  
   - Mini-derivation:  

   $$
   \nabla_x \mathcal{L} = \nabla_x U(x) + \sum_j \lambda_j \nabla_x g_j(x)
   $$

2. **Binmore, K.** *Game Theory: A Very Short Introduction*, Oxford University Press, 2007  
   - Multi-agent interactions, Nash equilibrium  
   - Mini-derivation:

   $$
   \frac{\partial U_1}{\partial x_1} = 0 \implies x_1^* = f(x_2)
   $$

3. **Courant, R.** *Differential Equations*, Springer, 1962  
   - Continuous-time dynamics, Euler & RK4 methods  
   - Mini-derivation:
...
4. **MIT OpenCourseWare**, *Multivariable Calculus & Differential Equations*  
   - Jacobian, gradient computation, partial derivatives  
   - Mini-derivation:

   $$
   \nabla_x U(x) = 
   \begin{bmatrix}
   \frac{\partial U}{\partial x_1} \\ \frac{\partial U}{\partial x_2} \\ \vdots
   \end{bmatrix}
   $$

5. **Rockafellar, R. T.** *Convex Analysis*, Princeton University Press, 1970  
   - Duality, constrained optimization theory  
   - Mini-derivation:

   $$
   \lambda_j \ge 0, \quad \lambda_j g_j(x^*) = 0
   $$

---

### Mini-Derivations / Proofs
- **Gradient of utility function**:

$$
\nabla_{x_i} U_i = \frac{\partial U_i}{\partial x_i}
$$

- **Euler approximation for dynamics**:
...
$$
|\Delta t| \ll \frac{1}{\max(|\lambda_i|)}
$$

- **Gradient-based Lagrangian update**:

$$
x_{t+1} = x_t + \eta \nabla_x \mathcal{L}(x_t, \lambda_t), \quad
\lambda_{t+1} = \lambda_t + \eta_\lambda g(x_t)
$$

- **Feasibility / constraint proof**:

$$
h_k(x^*) = 0, \quad g_j(x^*) \le 0
$$

- **Multi-agent Nash equilibrium condition**:

$$
\forall i, \quad x_i^* = \arg\max_{x_i} U_i(x_i, x_{-i}^*)
$$
